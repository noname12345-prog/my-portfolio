...
const getContribImages = (linkUrl, customImg, customIcon, type) => {
  ...
  if (isDiscord) {
    ...
    if (!finalIcon) finalIcon = "https://assets-global.website-files.com/6257adef93867e50d84d30e2/636e0a6a49cf127bf92de1e2_icon_clyde_blurple_RGB.png"; 
    if (finalIcon.includes("website-files.com")) {
      finalIcon = "https://cdn.prod.website-files.com/6257adef93867e50d84d30e2/636e0a6a49cf127bf92de1e2_icon_clyde_blurple_RGB.png";
    }
  } 
  else if (isRoblox) {
...
const injectStyles = () => {
  ...
  s.textContent = `
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .contrib-open-btn:active { transform: scale(0.98); }
    ...
    .pricing-card.highlight {
      border-color: ${C.blurple};
      box-shadow: 0 0 0 1px ${C.blurple}44;
    }
...
export default function Portfolio() {
  ...
  const [activeWork, setActiveWork] = useState(null); // Стейт для открытой карточки работы
  ...
  const [form, setForm] = useState({ title: "", category: "lua", desc: "", imageUrl: "", tags: "" });
  ...
  const loadWorksAndContribs = () => {
    try {
      const savedWorks = localStorage.getItem("syntaxia_works_v1");
      if (savedWorks) setWorks(JSON.parse(savedWorks));
      
      const savedContribs = localStorage.getItem("syntaxia_contribs_v1");
      if (savedContribs) setContribs(JSON.parse(savedContribs));
    } catch {}
    setLoading(false);
  };

  const saveWorks = (list) => {
    localStorage.setItem("syntaxia_works_v1", JSON.stringify(list));
    setWorks(list);
  };

  const saveContribs = (list) => {
    localStorage.setItem("syntaxia_contribs_v1", JSON.stringify(list));
    setContribs(list);
  };
  ...
  // Конвертация файла с ПК в Base64 формат (чтобы сохранять в браузере)
  ...
  const handleAdd = () => {
    ...
    setForm({ title: "", category: "lua", desc: "", imageUrl: "", tags: "" });
    setModal(null);
  };
  ...
  let memberCount = null; // New variable
  ...
  if (contribForm.type === "discord" && (!finalImg || !finalIcon)) {
    ...
    if (match) {
      try {
        ...
        if (data.guild) {
          memberCount = data.approximate_member_count; // Grab the count
          if (!finalIcon && data.guild.icon) {
            finalIcon = `https://cdn.discordapp.com/icons/${data.guild.id}/${data.guild.icon}.png?size=256`;
          }
          if (!finalImg && data.guild.splash) {
            finalImg = `https://cdn.discordapp.com/splashes/${data.guild.id}/${data.guild.splash}.png?size=512`;
          }
        }
      } catch (e) {
        console.warn("Failed to fetch discord invite data", e);
      }
    }
  }
  ...
  const contrib = {
      ...
      linkUrl: contribForm.linkUrl.trim(),
      imageUrl: finalImg,
      iconUrl: finalIcon,
      memberCount: memberCount // Save it to the object
    };
    saveContribs([...contribs, contrib]);
  ...
  const handleDeleteItem = () => {
    if (delType === "work") {
      saveWorks(works.filter(w => w.id !== delId));
    } else {
      saveContribs(contribs.filter(c => c.id !== delId));
    }
    ...
    <div style={{ minHeight: "100vh", background: C.bg }}>
      ...
      <nav style={{
        ...
        <span style={{
          ...
          fontSize: scrolled ? 13 : 15, color: C.white, letterSpacing: 1,
        ...
        <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
          ...
          <button
            className="admin-btn"
            onClick={() => isAdmin ? setIsAdmin(false) : setModal("login")}
            style={{
              background: isAdmin ? C.blurple + "22" : "none",
              border: `1px solid ${isAdmin ? C.blurple : C.border}`,
              color: isAdmin ? C.blurple : C.muted,
            }}>
            {isAdmin ? "ADMIN ✓" : "ADMIN"}
          </button>
        </div>
      ...
      <section id="home" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center", padding: "4rem 2rem", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", zIndex: 1 }} className="fade-in">
          ...
          <p style={{ fontSize: 16, color: C.muted, maxWidth: 420, lineHeight: 1.7, marginBottom: "2rem" }}>
            Roblox Lua & Haxe developer · Render artist · SFX designer
          </p>
      ...
      <section id="about" style={{ padding: "6rem 2rem", maxWidth: 720, margin: "0 auto" }}>
        <Label>About</Label>
        <h2 style={{ fontFamily: "'Space Mono', monospace", fontSize: "clamp(1.4rem, 3vw, 2rem)", color: C.white, marginBottom: "1.5rem", fontWeight: 700 }}>
          Hey, Im Syntaxia/Syn 👋
        </h2>
        <p style={{ fontSize: 15, color: C.muted, lineHeight: 1.9, marginBottom: "1.2rem" }}>
          Im a roblox <strong style={{ color: C.text }}>Luau</strong> developer as well as a <strong style={{ color: C.text }}>Haxe</strong> programmer, <strong style={{ color: C.text }}>render artist</strong> and <strong style={{ color: C.text }}>SFX designer</strong>. I can code almost anything, starting from simple scripts to complex game systems.
        </p>
        <p style={{ fontSize: 15, color: C.muted, lineHeight: 1.9, marginBottom: "1.2rem" }}>
          I've contributed to <strong style={{ color: C.text }}>UBA</strong> (10M+ visits). I'm always working on something new - whether it's a game, a visual piece, or a SFX I make for fun.
        </p>
      ...
      <section id="works" style={{ padding: "6rem 2rem", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 16, marginBottom: "2rem" }}>
          ...
          {isAdmin && (
            <button className="add-work-btn" onClick={() => setModal("add")} style={{ background: C.blurple, color: "#fff", border: "none", padding: "9px 18px", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 14, display: "flex", alignItems: "center", gap: 6 }}>
              + Add Work
            </button>
          )}
        </div>
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: "2rem" }}>
          {CATS.map(c => (
            <button key={c.id} className="filter-btn" onClick={() => setFilter(c.id)} style={{
              background: filter === c.id ? C.white : C.card,
              color: filter === c.id ? C.bg : C.muted,
              borderColor: filter === c.id ? C.white : C.border,
            }}>{c.label}</button>
          ))}
        </div>
        {loading ? (
          ...
          <div style={{ textAlign: "center", padding: "5rem 2rem", border: `1px dashed ${C.border}`, borderRadius: 12, color: C.dim }}>
            ...
            <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 13 }}>
              {isAdmin ? "No works yet - click '+ Add Work' to get started!" : "Nothing here yet. Check back soon."}
            </div>
          </div>
        ) : (
      ...
      <section id="contributions" style={{ padding: "6rem 2rem", maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ marginBottom: "2rem" }}>
          ...
          <h2 style={{ fontFamily: "'Space Mono', monospace", fontSize: "clamp(1.4rem, 3vw, 2rem)", color: C.white, fontWeight: 700 }}>
            Contributions
          </h2>
          {isAdmin && (
            <button className="add-work-btn" onClick={() => setModal("add_contrib")} style={{ background: "#22c55e", color: "#fff", border: "none", padding: "9px 18px", borderRadius: 8, cursor: "pointer", fontWeight: 600, fontSize: 14, display: "flex", alignItems: "center", gap: 6 }}>
              + Add Contribution
            </button>
          )}
        </div>
        ...
        {contribs.length === 0 ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: 20 }}>
            {contribs.map((item) => {
              const media = getContribImages(item.linkUrl, item.imageUrl, item.iconUrl, item.type);
              
              // Проверка, является ли фон ссылкой или файлом
              ...
              // Делаем ссылку безопасной для клика
              const safeLinkUrl = item.linkUrl?.match(/^https?:\/\//) ? item.linkUrl : `https://${item.linkUrl || ""}`;
              ...
              return (
                <div key={item.id} className="contrib-card" style={{ height: "100%" }}>
                  ...
                  <div style={{ 
                    height: 105, 
                    background: bgIsImage ? `url(${media.bg}) center/cover no-repeat` : media.bg, 
                  ...
                  <div style={{ padding: "0 16px 16px", position: "relative", display: "flex", flexDirection: "column", flexGrow: 1 }}>
                    ...
                    <div style={{ 
                      ...
                      width: 60, height: 60, borderRadius: item.type === "discord" ? "16px" : "8px", 
                      background: C.surface, border: `3px solid ${C.card}`,
                      display: "flex", alignItems: "center", justifyContent: "center",
                      overflow: "hidden", fontSize: "24px"
                    }}>
                      {iconIsImage ? (
                        <img src={media.icon} style={{ width: "100%", height: "100%", objectFit: "cover" }} alt="icon" />
                      ) : (
                        <span>{media.icon || "🎮"}</span>
                      )}
                    </div>

                    <div style={{ marginTop: "42px", marginBottom: "1.2rem", flexGrow: 1, textAlign: "left" }}>
                      <h4 style={{ fontFamily: "'Space Mono', monospace", fontSize: 15, color: C.white, fontWeight: 700, margin: "0 0 2px 0" }}>
                        {item.title}
                      </h4>
                      
                      {/* Discord Style Member Count */}
                      {item.type === "discord" && item.memberCount && (
                        <div style={{ display: "flex", alignItems: "center", gap: "6px", marginBottom: "8px" }}>
                          <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#22c55e" }}></span>
                          <span style={{ fontSize: 12, color: C.muted, fontFamily: "'Space Mono', monospace" }}>
                            {item.memberCount.toLocaleString()} members
                          </span>
                        </div>
                      )}

                      <p style={{ fontSize: 13, color: C.muted, lineHeight: 1.4, margin: 0 }}>
                        {item.desc || "Active partner project"}
                      </p>
                    </div>

                    {/* Кнопка Join теперь с безопасной ссылкой */}
                    <a href={safeLinkUrl} target="_blank" rel="noreferrer" className="contrib-open-btn">
                      Join
                    </a>
                  </div>
                </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 20 }}>
          {PRICING.map(p => (
            <div key={p.name} className={`pricing-card${p.highlight ? " highlight" : ""}`}>
              {p.highlight && (
                <div style={{ position: "absolute", top: 0, left: 0, right: 0, height: 3, background: `linear-gradient(90deg, ${C.blurple}, #a855f7)`, borderRadius: "16px 16px 0 0" }} />
              )}
              ...
              <button className="cta-btn" onClick={() => scrollTo("contact")} style={{
                width: "100%", background: p.highlight ? C.blurple : C.card2,
      ...
      <section id="contact" style={{ padding: "6rem 2rem", maxWidth: 600, margin: "0 auto" }}>
        ...
        <p style={{ color: C.muted, fontSize: 14, marginBottom: "2rem", lineHeight: 1.7 }}>
          Want to work together or have a question? Reach out on any of these platforms.
        </p>
        
        {/* Уменьшаем gap, чтобы элементы были плотнее */}
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}> 
          {[
            ...
            <a key={c.label} href={c.href} className="contact-link" style={{ display: "flex", alignItems: "center", gap: "12px", padding: "12px 16px" }}>
      ...
      {modal === "login" && (
        <Modal onClose={() => { setModal(null); setPw(""); setPwErr(false); }}>
          ...
          <input type="password" placeholder="Password" value={pw}
            ...
            style={inputStyle(pwErr)} autoFocus />
        ...
        <Modal onClose={() => setModal(null)} wide>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
              <button onClick={handleAdd} style={{ ...btnStyle(C.blurple, "#fff"), flex: 1 }}>Add Work</button>
      ...
      {modal === "add_contrib" && (
        <Modal onClose={() => setModal(null)} wide>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            ...
            <textarea placeholder="Description (e.g. Active partner project)" rows={2} value={contribForm.desc} onChange={e => setContribForm(f => ({ ...f, desc: e.target.value }))} style={{ ...inputStyle(), resize: "vertical" }} />
            <input placeholder="Link URL * (Discord invite or Roblox game URL)" value={contribForm.linkUrl} onChange={e => setContribForm(f => ({ ...f, linkUrl: e.target.value }))} style={inputStyle()} />
            
            <div style={{ display: "flex", gap: 10 }}>
              <input placeholder="Custom Background URL (Blank to auto-get)" value={contribForm.imageUrl} onChange={e => setContribForm(f => ({ ...f, imageUrl: e.target.value }))} style={{...inputStyle(), flex: 1}} />
            ...
            <div style={{ display: "flex", gap: 10 }}>
              <input placeholder="Custom Avatar URL (Blank to auto-get)" value={contribForm.iconUrl} onChange={e => setContribForm(f => ({ ...f, iconUrl: e.target.value }))} style={{...inputStyle(), flex: 1}} />
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
              <button onClick={handleAddContrib} style={{ ...btnStyle("#22c55e", "#fff"), flex: 1 }}>Add Contribution</button>
              <button onClick={() => setModal(null)} style={{ ...btnStyle(C.card2, C.muted), flex: 1 }}>Cancel</button>
            </div>
          </div>
        </Modal>
      ...
      {/* VIEW WORK MODAL (Открытие карточки) */}
      {modal === "view_work" && activeWork && (
        <Modal onClose={() => { setModal(null); setActiveWork(null); }} wide>
          {activeWork.imageUrl && (
            <img src={activeWork.imageUrl} alt={activeWork.title} style={{ width: "100%", maxHeight: "300px", objectFit: "cover", borderRadius: 8, marginBottom: 16 }} />
          )}
          <h3 style={{ fontFamily: "'Space Mono', monospace", color: C.white, fontSize: 20, marginBottom: 10 }}>{activeWork.title}</h3>
          
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
            <span style={{ fontSize: 12, color: C.blurple, background: C.blurple + "22", padding: "4px 12px", borderRadius: 20, fontFamily: "'Space Mono', monospace", fontWeight: 600 }}>
              {CATS.find(c => c.id === activeWork.category)?.label || activeWork.category}
            </span>
            {(activeWork.tags || []).map(t => (
              <span key={t} style={{ fontSize: 12, color: C.dim, background: C.card2, padding: "4px 12px", borderRadius: 20, fontFamily: "'Space Mono', monospace" }}>{t}</span>
            ))}
          </div>

          {activeWork.desc && (
            <p style={{ color: C.muted, fontSize: 14, lineHeight: 1.6, marginBottom: 24, whiteSpace: "pre-wrap" }}>{activeWork.desc}</p>
          )}
    ...
    <div className="work-card" onClick={onClick}>
      <div style={{ height: 160, background: C.card2, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", position: "relative" }}>
        {work.imageUrl ? (
          ...
          <div style={{ fontSize: 40, opacity: 0.3 }}>
            {{ lua: "🎮", haxe: "⚙️", render: "🎨", sfx: "🔊" }[work.category] || "📁"}
          </div>
        )}
        <span style={{ position: "absolute", top: 10, left: 10, background: color + "22", color, border: `1px solid ${color}44`, padding: "3px 10px", borderRadius: 20, fontSize: 11, fontFamily: "'Space Mono', monospace" }}>
          {CATS.find(c => c.id === work.category)?.label || work.category}
        </span>
        {isAdmin && (
          <button className="del-btn" onClick={onDelete} style={{ position: "absolute", top: 8, right: 8, background: "#ef444422", color: "#ef4444", border: "1px solid #ef444444", borderRadius: 6, width: 28, height: 28, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, transition: "background 0.15s" }}>🗑</button>
        )}
      </div>
      <div style={{ padding: "1rem 1.2rem" }}>
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {(work.tags || []).map(t => (
            <span key={t} style={{ fontSize: 11, color: C.dim, background: C.card2, padding: "2px 8px", borderRadius: 4, fontFamily: "'Space Mono', monospace" }}>{t}</span>
          ))}
...
function Label({ children }) {
  return (
    <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: C.blurple, letterSpacing: 3, textTransform: "uppercase", marginBottom: 8 }}>
      {children}
    </div>
  );
}
